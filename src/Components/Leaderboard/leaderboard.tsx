"use client"

import { useState, useEffect, useCallback } from "react"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
  PaginationItem,
  PaginationLink,
  PaginationNext,
  PaginationPrevious,
} from "@/components/ui/pagination"
import { Search, Trophy, Medal, Award, RefreshCw, Sparkles, Calendar } from "lucide-react"

// Enhanced sample data with week information
const initialLeaderboardData = [
  // Week 1
  { id: 1, name: "Alex Johnson", score: 98, time: "12:45", date: "2023-11-15", week: 1 },
  { id: 2, name: "Maria Garcia", score: 95, time: "13:20", date: "2023-11-15", week: 1 },
  { id: 3, name: "Sam Taylor", score: 92, time: "14:10", date: "2023-11-15", week: 1 },
  { id: 4, name: "Priya Patel", score: 90, time: "11:55", date: "2023-11-15", week: 1 },

  // Week 2
  { id: 5, name: "David Kim", score: 88, time: "15:30", date: "2023-11-22", week: 2 },
  { id: 6, name: "Emma Wilson", score: 85, time: "12:15", date: "2023-11-22", week: 2 },
  { id: 7, name: "Omar Hassan", score: 82, time: "13:45", date: "2023-11-22", week: 2 },
  { id: 8, name: "Sophia Chen", score: 80, time: "14:20", date: "2023-11-22", week: 2 },

  // Week 3
  { id: 9, name: "Lucas Müller", score: 78, time: "16:05", date: "2023-11-29", week: 3 },
  { id: 10, name: "Aisha Okafor", score: 75, time: "11:30", date: "2023-11-29", week: 3 },
  { id: 11, name: "James Smith", score: 73, time: "12:50", date: "2023-11-29", week: 3 },
  { id: 12, name: "Fatima Ali", score: 70, time: "13:15", date: "2023-11-29", week: 3 },

  // Week 4
  { id: 13, name: "Alex Johnson", score: 96, time: "11:45", date: "2023-12-06", week: 4 },
  { id: 14, name: "Maria Garcia", score: 93, time: "12:30", date: "2023-12-06", week: 4 },
  { id: 15, name: "Sam Taylor", score: 90, time: "13:15", date: "2023-12-06", week: 4 },
  { id: 16, name: "Priya Patel", score: 88, time: "14:20", date: "2023-12-06", week: 4 },

  // Week 5
  { id: 17, name: "David Kim", score: 94, time: "10:30", date: "2023-12-13", week: 5 },
  { id: 18, name: "Emma Wilson", score: 91, time: "11:45", date: "2023-12-13", week: 5 },
  { id: 19, name: "Omar Hassan", score: 89, time: "12:30", date: "2023-12-13", week: 5 },
  { id: 20, name: "Sophia Chen", score: 87, time: "13:15", date: "2023-12-13", week: 5 },
]

export default function Leaderboard() {
  // React state hooks for component state
  const [leaderboardData, setLeaderboardData] = useState(initialLeaderboardData)
  const [searchQuery, setSearchQuery] = useState("")
  const [currentPage, setCurrentPage] = useState(1)
  const [sortBy, setSortBy] = useState("score")
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc")
  const [isLoading, setIsLoading] = useState(false)
  const [highlightedRow, setHighlightedRow] = useState<number | null>(null)
  const [showAnimation, setShowAnimation] = useState(false)
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null)
  const [selectedWeek, setSelectedWeek] = useState<string>("total")
  const itemsPerPage = 10

  // Get unique weeks from data
  const weeks = Array.from(new Set(leaderboardData.map((item) => item.week))).sort((a, b) => a - b)

  // Calculate total points for each participant
  const calculateTotalPoints = useCallback(() => {
    const participants: { [key: string]: { id: number; name: string; score: number; time: string; date: string } } = {}

    leaderboardData.forEach((entry) => {
      if (!participants[entry.name]) {
        participants[entry.name] = {
          id: entry.id,
          name: entry.name,
          score: entry.score,
          time: entry.time,
          date: entry.date,
        }
      } else {
        participants[entry.name].score += entry.score
      }
    })

    return Object.values(participants)
  }, [leaderboardData])

  // Trigger initial animation
  useEffect(() => {
    setShowAnimation(true)
    const timer = setTimeout(() => setShowAnimation(false), 1000)
    return () => clearTimeout(timer)
  }, [])

  // Memoized filtering and sorting function using useCallback
  const getProcessedData = useCallback(() => {
    // First, filter by week if needed
    let dataToProcess = [...leaderboardData]

    if (selectedWeek !== "total") {
      const weekNumber = Number.parseInt(selectedWeek.replace("week", ""))
      dataToProcess = dataToProcess.filter((entry) => entry.week === weekNumber)
    } else {
      // For total points, aggregate scores by participant
      dataToProcess = calculateTotalPoints()
    }

    // Then filter by search query
    return dataToProcess
      .filter((entry) => entry.name.toLowerCase().includes(searchQuery.toLowerCase()))
      .sort((a, b) => {
        let comparison = 0

        if (sortBy === "score") {
          comparison = a.score - b.score
        } else if (sortBy === "time") {
          comparison = a.time.localeCompare(b.time)
        } else if (sortBy === "date") {
          comparison = new Date(a.date).getTime() - new Date(b.date).getTime()
        } else if (sortBy === "name") {
          comparison = a.name.localeCompare(b.name)
        }

        return sortOrder === "desc" ? -comparison : comparison
      })
  }, [leaderboardData, searchQuery, sortBy, sortOrder, selectedWeek, calculateTotalPoints])

  // Get filtered and sorted data
  const filteredData = getProcessedData()

  // Paginate data
  const indexOfLastItem = currentPage * itemsPerPage
  const indexOfFirstItem = indexOfLastItem - itemsPerPage
  const currentItems = filteredData.slice(indexOfFirstItem, indexOfLastItem)
  const totalPages = Math.ceil(filteredData.length / itemsPerPage)

  // Reset to first page when search query or selected week changes
  useEffect(() => {
    setCurrentPage(1)
  }, [searchQuery, selectedWeek])

  // Simulate data refresh
  const refreshData = () => {
    setIsLoading(true)
    setShowAnimation(true)
    // Simulate API call delay
    setTimeout(() => {
      // In a real app, you would fetch fresh data here
      // For demo, we'll just randomize some scores
      const updatedData = leaderboardData.map((entry) => ({
        ...entry,
        score: Math.max(60, Math.min(100, entry.score + Math.floor(Math.random() * 11) - 5)),
      }))
      setLeaderboardData(updatedData)
      setIsLoading(false)

      // Reset animation after a delay
      setTimeout(() => setShowAnimation(false), 1000)
    }, 800)
  }

  // Handle sort header click
  const handleSortClick = (column: string) => {
    if (sortBy === column) {
      // Toggle sort order if clicking the same column
      setSortOrder(sortOrder === "asc" ? "desc" : "asc")
    } else {
      // Set new sort column and default to descending for score, ascending for others
      setSortBy(column)
      setSortOrder(column === "score" ? "desc" : "asc")
    }
  }

  // Get sort indicator
  const getSortIndicator = (column: string) => {
    if (sortBy !== column) return null
    return sortOrder === "asc" ? " ↑" : " ↓"
  }

  // Get rank icon based on position
  const getRankIcon = (index: number) => {
    const position = indexOfFirstItem + index + 1
    if (position === 1) return <Trophy className="h-5 w-5 text-yellow-500 animate-pulse" />
    if (position === 2) return <Medal className="h-5 w-5 text-gray-400" />
    if (position === 3) return <Medal className="h-5 w-5 text-amber-700" />
    return <span className="font-medium">{position}</span>
  }

  // Calculate stats based on selected week or total
  const getStats = useCallback(() => {
    let dataForStats = [...leaderboardData]

    if (selectedWeek !== "total") {
      const weekNumber = Number.parseInt(selectedWeek.replace("week", ""))
      dataForStats = dataForStats.filter((entry) => entry.week === weekNumber)
    } else {
      dataForStats = calculateTotalPoints()
    }

    const topScore = Math.max(...dataForStats.map((entry) => entry.score))
    const topScorer = dataForStats.find((entry) => entry.score === topScore)?.name
    const averageScore = Math.round(dataForStats.reduce((sum, entry) => sum + entry.score, 0) / dataForStats.length)

    let latestDate, participantsOnLatestDate

    if (selectedWeek !== "total") {
      latestDate = new Date(Math.max(...dataForStats.map((entry) => new Date(entry.date).getTime())))
      participantsOnLatestDate = dataForStats.filter(
        (entry) => new Date(entry.date).toDateString() === latestDate.toDateString(),
      ).length
    } else {
      latestDate = new Date(Math.max(...leaderboardData.map((entry) => new Date(entry.date).getTime())))
      participantsOnLatestDate = dataForStats.length
    }

    return { topScore, topScorer, averageScore, latestDate, participantsOnLatestDate }
  }, [leaderboardData, selectedWeek, calculateTotalPoints])

  const { topScore, topScorer, averageScore, latestDate, participantsOnLatestDate } = getStats()

  // Menu items for sidebar
  const menuItems = [
    {
      name: "Home",
      color: "#000000",
      icon: (
        <svg
          width="35px"
          height="35px"
          viewBox="-4.5 0 265 265"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          preserveAspectRatio="xMidYMid"
          fill="#000000"
        >
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            {" "}
            <g>
              {" "}
              <path
                d="M199.061185,207.634845 C199.085502,217.03865 202.381002,226.654917 202.377398,236.058845 C202.373721,245.604163 199.070969,254.937147 199.100185,264.482845 C151.689185,264.475845 104.277185,264.482845 56.8671849,264.478845 C56.8899646,255.095091 52.4489679,245.441856 52.4451431,236.058845 C52.4415107,226.494356 56.8750029,217.200091 56.8941849,207.634845 C104.284185,207.649845 151.671185,207.638845 199.061185,207.634845 Z"
                fill="#3BAB59"
              >
                {" "}
              </path>{" "}
              <path
                d="M144.20343,50.8391621 C150.788967,44.2673942 161.302534,40.8049071 167.812585,34.1582447 C194.787585,61.2992447 221.876585,88.3342447 248.915585,115.419245 C251.351585,117.927245 254.321585,120.235245 255.272585,123.774245 C256.201585,126.077245 255.917585,128.586245 255.959585,131.007245 C255.963585,170.664245 255.955585,210.317245 255.965585,249.970245 C256.564585,257.795245 249.104585,265.104245 241.313585,264.482245 C227.241585,264.482245 213.172585,264.486245 199.099585,264.482245 C199.042585,245.533245 199.110585,226.584245 199.061585,207.635245 C199.095585,186.802245 199.076585,165.973245 199.069585,145.144245 C181.506585,127.609245 163.981585,110.034245 146.430585,92.4912447 C140.307585,86.3152447 134.089585,80.2292447 128.023585,74.0002447 C131.349585,70.8172447 140.762034,54.2733623 144.20343,50.8391621 Z"
                fill="#4586F7"
              >
                {" "}
              </path>{" "}
              <path
                d="M56.8487849,145.220645 C56.9277849,166.022645 56.8407849,186.829645 56.8937849,207.634645 C56.8557849,226.583645 56.9127849,245.529645 56.8677849,264.478645 C42.0597849,264.471645 27.2517849,264.482645 12.4437849,264.471645 C6.25978491,263.694645 0.814784906,258.488645 0.0267849064,252.254645 C-0.0222150936,235.534645 0.0117849064,218.812645 0.0157849064,202.094645 C9.88278491,192.319645 16.0386327,179.624241 25.8716327,169.814241 C34.9806327,160.667241 47.7817849,154.409645 56.8487849,145.220645 Z"
                fill="#FDC00D"
              >
                {" "}
              </path>{" "}
              <path
                d="M127.246385,0.0255447371 C128.470385,0.177544737 129.925385,-0.304455263 131.013385,0.381544737 C133.968385,0.957544737 136.559385,2.66654474 138.637385,4.79954474 C147.465385,13.6475447 156.301385,22.4875447 165.138385,31.3275447 C166.024385,32.2745447 167.058385,33.0895447 167.812385,34.1575447 C157.900385,44.2785447 147.802385,54.2135447 137.818385,64.2625447 C134.544385,67.4985447 131.349385,70.8175447 128.023385,74.0005447 C123.635385,78.1345447 119.482385,82.5105447 115.185385,86.7395447 C95.7473849,106.241545 76.2413849,125.672545 56.8483849,145.220545 C47.7813849,154.409545 38.6043849,163.495545 29.4943849,172.642545 C19.6623849,182.452545 9.88238491,192.319545 0.0153849064,202.094545 C-0.0186150936,180.660545 0.0153849064,159.224545 0.00838490642,137.793545 C0.0313849064,133.747545 -0.0566150936,129.700545 0.0833849064,125.657545 C0.0423849064,124.838545 0.659384906,124.190545 0.621384906,123.372545 C1.19438491,122.410545 1.70538491,121.409545 2.26238491,120.439545 C3.66438491,118.575545 5.40738491,117.014545 7.03338491,115.351545 C43.8143849,78.4945447 80.6063849,41.6455447 117.398385,4.79554474 C118.903385,3.21554474 120.626385,1.77954474 122.722385,1.06354474 C124.192385,0.571544737 125.647385,-0.144455263 127.246385,0.0255447371 Z"
                fill="#EC5043"
              >
                {" "}
              </path>{" "}
            </g>{" "}
          </g>
        </svg>
      ),
    },
    {
      name: "Team",
      color: "#EA4335",
      icon: (
        <svg
          height="35px"
          width="35px"
          version="1.1"
          id="_x32_"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 512 512"
          xmlSpace="preserve"
          fill="#EA4335"
          stroke="#EA4335"
        >
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            <style type="text/css"></style>
            <g>
              <path
                className="st0"
                d="M435.95,287.525c32.51,0,58.87-26.343,58.87-58.853c0-32.51-26.361-58.871-58.87-58.871 c-32.502,0-58.863,26.361-58.863,58.871C377.088,261.182,403.448,287.525,435.95,287.525z"
              />
              <path
                className="st0"
                d="M511.327,344.251c-2.623-15.762-15.652-37.822-25.514-47.677c-1.299-1.306-7.105-1.608-8.673-0.636 c-11.99,7.374-26.074,11.714-41.19,11.714c-15.099,0-29.184-4.34-41.175-11.714c-1.575-0.972-7.373-0.67-8.672,0.636 c-2.757,2.757-5.765,6.427-8.698,10.683c7.935,14.94,14.228,30.81,16.499,44.476c2.27,13.7,1.533,26.67-2.138,38.494 c13.038,4.717,28.673,6.787,44.183,6.787C476.404,397.014,517.804,382.987,511.327,344.251z"
              />
              <path
                className="st0"
                d="M254.487,262.691c52.687,0,95.403-42.716,95.403-95.402c0-52.67-42.716-95.386-95.403-95.386 c-52.678,0-95.378,42.716-95.378,95.386C159.109,219.975,201.808,262.691,254.487,262.691z"
              />
              <path
                className="st0"
                d="M335.269,277.303c-2.07-2.061-11.471-2.588-14.027-1.006c-19.448,11.966-42.271,18.971-66.755,18.971 c-24.466,0-47.3-7.005-66.738-18.971c-2.555-1.583-11.956-1.055-14.026,1.006c-16.021,16.004-37.136,51.782-41.384,77.288 c-10.474,62.826,56.634,85.508,122.148,85.508c65.532,0,132.639-22.682,122.165-85.508 C372.404,329.085,351.289,293.307,335.269,277.303z"
              />
              <path
                className="st0"
                d="M76.049,287.525c32.502,0,58.862-26.343,58.862-58.853c0-32.51-26.36-58.871-58.862-58.871 c-32.511,0-58.871,26.361-58.871,58.871C17.178,261.182,43.538,287.525,76.049,287.525z"
              />
              <path
                className="st0"
                d="M115.094,351.733c2.414-14.353,9.225-31.253,17.764-46.88c-2.38-3.251-4.759-6.083-6.955-8.279 c-1.299-1.306-7.097-1.608-8.672-0.636c-11.991,7.374-26.076,11.714-41.182,11.714c-15.108,0-29.202-4.34-41.183-11.714 c-1.568-0.972-7.382-0.67-8.681,0.636c-9.887,9.854-22.882,31.915-25.514,47.677c-6.468,38.736,34.924,52.762,75.378,52.762 c14.437,0,29.016-1.777,41.459-5.84C113.587,379.108,112.757,365.835,115.094,351.733z"
              />
            </g>
          </g>
        </svg>
      ),
    },
    {
      name: "Events",
      color: "#4285F4",
      icon: (
        <svg
          height="35px"
          width="35px"
          version="1.1"
          id="_x32_"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 512.00 512.00"
          xmlSpace="preserve"
          fill="#4285F4"
          stroke="#4285F4"
          strokeWidth="0.00512"
        >
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            {" "}
            <style type="text/css"></style>
            <g>
              <path
                className="st0"
                d="M118.612,89.298c9.483,0,17.177-7.686,17.177-17.168v-54.96C135.788,7.687,128.094,0,118.612,0 c-9.482,0-17.176,7.687-17.176,17.169v54.96C101.436,81.611,109.13,89.298,118.612,89.298z"
              />
              <path
                className="st0"
                d="M255.992,89.298c9.483,0,17.176-7.686,17.176-17.168v-54.96C273.169,7.687,265.475,0,255.992,0 c-9.483,0-17.169,7.687-17.169,17.169v54.96C238.824,81.611,246.51,89.298,255.992,89.298z"
              />
              <path
                className="st0"
                d="M393.372,89.298c9.49,0,17.176-7.686,17.176-17.168v-54.96C410.549,7.687,402.863,0,393.372,0 c-9.482,0-17.169,7.687-17.169,17.169v54.96C376.204,81.611,383.89,89.298,393.372,89.298z"
              />
              <path
                className="st0"
                d="M196.576,275.986h23.082c1.611,0,2.683-1.071,2.683-2.682v-20.931c0-1.079,0.539-1.611,1.611-1.611h61.142 v0.802l-59.578,151.904c-0.802,1.881,0,2.953,1.611,2.953h25.495c2.143,0,2.683-0.54,3.492-2.413l60.379-151.635 c0.54-1.341,0.81-2.683,0.81-4.834v-20.931c0-1.611-1.072-2.682-2.683-2.682H196.576c-1.611,0-2.682,1.071-2.682,2.682v46.696 C193.894,274.915,194.965,275.986,196.576,275.986z"
              />
              <path
                className="st0"
                d="M424.286,44.899v27.23c0,17.038-13.861,30.906-30.914,30.906c-17.038,0-30.906-13.869-30.906-30.906v-27.23 h-75.559v27.23c0,17.038-13.869,30.906-30.914,30.906c-17.038,0-30.907-13.869-30.907-30.906v-27.23h-75.558v27.23 c0,17.038-13.87,30.906-30.915,30.906c-17.038,0-30.914-13.869-30.914-30.906v-27.23H17.866V512h476.267V44.899H424.286z M450.86,468.735H61.132V143.116H450.86V468.735z"
              />
            </g>
          </g>
        </svg>
      ),
    },
    {
      name: "Achievements",
      color: "#FBBC05",
      icon: (
        <svg
          height="35px"
          width="35px"
          version="1.1"
          id="_x32_"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 512 512"
          xmlSpace="preserve"
          fill="#FBBC05"
        >
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            <style type="text/css"></style>
            <g>
              <path
                className="st0"
                d="M348.375,384.758c-12.811-25.137-32.785-44.594-56.582-54.492v-38.5l0.047-9.133c-0.016,0-0.031,0-0.047,0.004 v-0.242c-11.588,2.262-23.551,3.481-35.791,3.481c-11.369,0-22.476-1.094-33.291-3.055c-0.752-0.152-1.516-0.262-2.264-0.426v0.043 c-0.08-0.016-0.16-0.028-0.24-0.043v47.871c-12.209,5.078-23.393,12.695-33.137,22.293c-0.348,0.34-0.705,0.66-1.049,1.004 c-1.072,1.082-2.1,2.219-3.133,3.348c-0.705,0.77-1.426,1.512-2.115,2.305c-0.61,0.703-1.184,1.442-1.78,2.156 c-1.07,1.289-2.14,2.574-3.168,3.918c-0.088,0.117-0.17,0.238-0.26,0.355c-4.392,5.789-8.406,12.078-11.939,18.875h0.131 c-0.043,0.082-0.09,0.16-0.131,0.238H348.375z"
              />
              <polygon
                className="st0"
                points="115.046,416 115.046,511.371 115.044,511.758 115.046,511.758 115.046,512 396.957,512 396.957,416 "
              />
              <path
                className="st0"
                d="M498.331,29.387c-8.027-9.094-19.447-14.312-31.328-14.312h-47.744V0.27V0.242l0,0V0H92.742v15.074H44.999 c-11.887,0-23.306,5.218-31.336,14.312C3.906,40.442-0.305,56.43,1.775,74.465c0.369,7.922,4.367,49.316,47.211,78.77 c24.732,17.008,48.424,24.629,69.44,27.938c29.008,45.328,79.76,75.398,137.576,75.398c57.805,0,108.558-30.07,137.568-75.398 c21.016-3.305,44.709-10.93,69.445-27.938c42.84-29.453,46.842-70.848,47.211-78.77C512.304,56.43,508.093,40.442,498.331,29.387z M476.238,71.016l-0.125,0.852l-0.002,1.031c-0.029,1.246-1.115,30.656-32.447,52.195c-8.976,6.172-17.635,10.719-26.041,14.184 c-1.836,0.711-3.668,1.43-5.553,2.043c4.664-15.184,7.19-31.297,7.19-48.008V49.226h47.744c1.498,0,3.711,0.481,5.726,2.758 C476.009,55.703,477.288,62.637,476.238,71.016z M253.964,155.219l-33.658,18.73c-1.422,0.793-3.174,0.688-4.49-0.274 c-1.312-0.949-1.959-2.586-1.644-4.18l7.412-37.801c0.279-1.418-0.193-2.883-1.254-3.863l-28.213-26.23 c-1.191-1.106-1.633-2.805-1.129-4.352s1.859-2.664,3.474-2.859l38.236-4.633c1.436-0.172,2.678-1.078,3.291-2.391l16.219-34.93 c0.687-1.477,2.162-2.422,3.795-2.422c1.625,0,3.102,0.945,3.787,2.422l16.22,34.93c0.612,1.312,1.854,2.219,3.289,2.391 l38.236,4.633c1.615,0.195,2.971,1.312,3.474,2.859c0.504,1.547,0.063,3.246-1.127,4.352l-28.215,26.23 c-1.059,0.98-1.541,2.445-1.26,3.863l7.418,37.801c0.313,1.594-0.328,3.23-1.648,4.18c-1.316,0.961-3.06,1.066-4.486,0.274 l-33.664-18.73C256.769,154.52,255.23,154.52,253.964,155.219z M68.331,125.094c-31.326-21.539-32.41-50.949-32.438-52.016 l-0.006-1.035l-0.131-1.027c-1.043-8.379,0.232-15.312,3.516-19.031c2.01-2.277,4.222-2.758,5.726-2.758h47.742v44.086 c0,14.246,1.928,28.02,5.357,41.192c0.559,2.308,1.076,4.629,1.725,6.926C89.732,137.801,79.257,132.602,68.331,125.094z"
              />
            </g>
          </g>
        </svg>
      ),
    },
    {
      name: "Blogs",
      color: "#34A853",
      icon: (
        <svg
          fill="#34A853"
          width="35px"
          height="35px"
          viewBox="0 0 24 24"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            <path d="M19.0863 5.6694v-2.875a.6101.6101 0 0 0-.6114-.6073H5.5235a.6101.6101 0 0 0-.6156.6074v2.834l8.3905-3.052a.5938.5938 0 0 1 .7422.3895l.5448 1.4981zM23.582 7.343l-8.8032-2.3683.7518 2.066h5.5837a.6142.6142 0 0 1 .6128.6128v8.795l2.247-8.366a.5924.5924 0 0 0-.3921-.7395zM2.27 7.6535a.6156.6156 0 0 1 .613-.6128h12.1546l-1.2693-3.4905a.5951.5951 0 0 0-.7435-.3827L.3674 7.7748a.5938.5938 0 0 0-.3227.775l2.2253 6.112zm0 0a.6156.6156 0 0 1 .613-.6128h12.1546l-1.2693-3.4905a.5951.5951 0 0 0-.7435-.3827L.3674 7.7748a.5938.5938 0 0 0-.3227.775l2.2253 6.112zM21.2668 21.2a.6142.6142 0 0 1-.6129.6129H3.3445a.6142.6142 0 0 1-.6129-.6129V8.1153a.6142.6142 0 0 1 .6129-.6129h17.3093a.6142.6142 0 0 1 .6129.6129zm-3.0315-3.4196v-1.1958a.0804.0804 0 0 0-.0804-.0803h-5.3726v1.3619h5.3726a.0817.0817 0 0 0 .0804-.0858zm.817-2.5876v-1.2012a.0804.0804 0 0 0-.0803-.0817h-6.1897v1.3619h6.1897a.079.079 0 0 0 .0804-.0817zm-.817-2.5875v-1.2067a.0804.0804 0 0 0-.0804-.0803h-5.3726v1.3619h5.3726a.0817.0817 0 0 0 .0804-.079zm-10.086 1.4395v1.2256h1.7704c-.1457.749-.8049 1.2925-1.7704 1.2925a1.976 1.976 0 0 1 0-3.9495 1.7704 1.7704 0 0 1 1.2529.4903l.9342-.9329a3.1405 3.1405 0 0 0-2.1871-.8525 3.2685 3.2685 0 1 0 0 6.537c1.8889 0 3.1323-1.3279 3.1323-3.1977a3.9576 3.9576 0 0 0-.0518-.6183Z" />
          </g>
        </svg>
      ),
    },
    {
      name: "Leaderboard",
      color: "#FF6D00",
      icon: (
        <svg width="40px" height="40px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g id="SVGRepo_bgCarrier" strokeWidth="0" />
          <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round" />
          <g id="SVGRepo_iconCarrier">
            <path
              d="M20.4 21H15V18.1C15 17.7686 15.2686 17.5 15.6 17.5H20.4C20.7314 17.5 21 17.7686 21 18.1V20.4C21 20.7314 20.7314 21 20.4 21Z"
              stroke="#4285F4"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M9 21V16.1C9 15.7686 8.73137 15.5 8.4 15.5H3.6C3.26863 15.5 3 15.7686 3 16.1V20.4C3 20.7314 3.26863 21 3.6 21H9Z"
              stroke="#34A853"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M15 21H9V12.6C9 12.2686 9.26863 12 9.6 12H14.4C14.7314 12 15 12.2686 15 12.6V21Z"
              stroke="#EA4335"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M10.8056 5.11325L11.7147 3.1856C11.8314 2.93813 12.1686 2.93813 12.2853 3.1856L13.1944 5.11325L15.2275 5.42427C15.4884 5.46418 15.5923 5.79977 15.4035 5.99229L13.9326 7.4917L14.2797 9.60999C14.3243 9.88202 14.0515 10.0895 13.8181 9.96099L12 8.96031L10.1819 9.96099C9.94851 10.0895 9.67568 9.88202 9.72026 9.60999L10.0674 7.4917L8.59651 5.99229C8.40766 5.79977 8.51163 5.46418 8.77248 5.42427L10.8056 5.11325Z"
              stroke="#FBBC05"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </g>
        </svg>
      ),
    },
  ]

  return (
    <div className="flex">
      {/* Sidebar */}
      <div className="fixed bg-black text-white h-screen p-3 flex flex-col items-center transition-all duration-300 border-r-2 border-white z-20">
        {/* Logo */}
        <a href="#" className="text-white mb-4 focus:outline-none">
          <svg
            width="60px"
            height="60px"
            viewBox="0 -65.5 256 256"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            preserveAspectRatio="xMidYMid"
            fill="#000000"
            stroke="#000000"
            strokeWidth="5.12"
          >
            <g id="SVGRepo_bgCarrier" strokeWidth="0" />
            <g
              id="SVGRepo_tracerCarrier"
              strokeLinecap="round"
              strokeLinejoin="round"
              stroke="#CCCCCC"
              strokeWidth="4.608"
            />
            <g id="SVGRepo_iconCarrier">
              {" "}
              <g>
                {" "}
                <path
                  d="M184.31481,67.7044587 C197.78381,59.9684587 211.21681,52.1694587 224.73181,44.5134587 C237.55981,37.2454587 252.65281,43.9484587 255.56081,58.0924587 C257.24381,66.2744587 253.59081,75.1134587 246.20381,79.4184587 C221.98581,93.5314587 197.73281,107.588459 173.35881,121.430459 C165.81481,125.714459 158.04381,124.926459 151.16881,119.676459 C144.18481,114.346459 141.98381,106.994459 143.62181,98.4374587 C145.49181,91.9234587 149.77181,87.5684587 155.60881,84.3104587 C165.25981,78.9244587 174.75381,73.2544587 184.31481,67.7044587"
                  fill="#FABC05"
                >
                  {" "}
                </path>{" "}
                <path
                  d="M194.20341,62.0789587 C180.76841,54.2829587 167.29941,46.5479587 153.91141,38.6729587 C141.20241,31.1969587 139.46241,14.7749587 150.25641,5.18295871 C156.50041,-0.364041286 165.98141,-1.62104129 173.40341,2.62395871 C197.73541,16.5409587 222.03541,30.5169587 246.20941,44.7039587 C253.69141,49.0949587 256.89341,56.2199587 255.78641,64.7989587 C254.66141,73.5109587 249.39541,79.0929587 241.16641,81.9539587 C234.58941,83.5919587 228.67841,82.0619587 222.93841,78.6359587 C213.44741,72.9709587 203.79041,67.5829587 194.20341,62.0789587"
                  fill="#109D58"
                >
                  {" "}
                </path>{" "}
                <path
                  d="M71.7518102,56.5628587 C63.1308102,61.4608587 54.5048102,66.3498587 45.8928102,71.2628587 C40.8548102,74.1368587 35.8728102,77.1088587 30.8088102,79.9348587 C20.6058102,85.6298587 8.48381017,82.2918587 2.69881017,72.2608587 C-2.82218983,62.6888587 0.35081017,50.2788587 10.1768102,44.5428587 C34.2018102,30.5198587 58.2888102,16.5988587 82.4628102,2.83385871 C89.8768102,-1.38814129 97.5688102,-0.857141286 104.42481,4.18985871 C111.66081,9.51685871 114.02981,17.0128587 112.40481,25.8008587 C111.39681,27.9268587 110.79481,30.4198587 109.28581,32.0948587 C106.83081,34.8198587 104.04081,37.4288587 100.93481,39.3448587 C91.3228102,45.2718587 81.4958102,50.8498587 71.7518102,56.5628587"
                  fill="#E94436"
                >
                  {" "}
                </path>{" "}
                <path
                  d="M61.8670102,62.0569587 C70.4200102,67.0729587 78.9670102,72.0999587 87.5280102,77.1019587 C92.5350102,80.0279587 97.6000102,82.8569587 102.57901,85.8279587 C112.61301,91.8179587 115.78401,103.983959 109.98901,114.008959 C104.45901,123.576959 92.1260102,127.034959 82.2450102,121.391959 C58.0880102,107.596959 33.9890102,93.6989587 9.98101017,79.6459587 C2.61801017,75.3359587 -0.76798983,68.4089587 0.17501017,59.9479587 C1.17001017,51.0169587 6.47701017,45.2179587 14.9000102,42.2309587 C17.2450102,42.0419587 19.7050102,41.3159587 21.9110102,41.7859587 C25.4980102,42.5499587 29.1530102,43.6609587 32.3650102,45.3929587 C42.3040102,50.7529587 52.0480102,56.4749587 61.8670102,62.0569587"
                  fill="#4385F3"
                >
                  {" "}
                </path>{" "}
              </g>{" "}
            </g>
          </svg>
        </a>

        {/* Navigation Menu */}
        <nav className="flex flex-col gap-6 mt-6">
          {menuItems.map((item, index) => (
            <div
              key={index}
              className="relative group"
              onMouseEnter={() => setHoveredIndex(index)}
              onMouseLeave={() => setHoveredIndex(null)}
            >
              {/* Icon Link */}
              <a
                href="#"
                className="flex items-center justify-center w-14 h-14 rounded transition-all hover:bg-gray-900"
              >
                <span style={{ fontSize: "25px", color: item.color }}>{item.icon}</span>
              </a>

              {/* Tooltip */}
              <span
                className={`absolute left-14 top-1/2 transform -translate-y-1/2 transition-all duration-200 bg-gray-800 text-white text-sm rounded-lg px-3 py-1 shadow-md whitespace-nowrap z-50 ${hoveredIndex === index ? "opacity-100 translate-x-2" : "opacity-0"}`}
              >
                {item.name}
              </span>
            </div>
          ))}
        </nav>
      </div>

      {/* Main Content */}
      <div className="flex-1 min-h-screen bg-black transition-all ml-24 overflow-auto">
        <div className="container mx-auto py-8 px-4">
          <div
            className={`flex flex-col md:flex-row justify-between items-start md:items-center mb-8 ${showAnimation ? "animate-fadeIn" : ""}`}
          >
            <h1 className="text-3xl font-bold mb-4 md:mb-0 relative">
              <span className="google-colored-text">Quiz Leaderboard</span>
              <span className="absolute -bottom-2 left-0 w-1/3 h-1 bg-gradient-to-r from-[#4285F4] via-[#EA4335] to-[#34A853] rounded-full"></span>
            </h1>
            <div className="flex flex-col md:flex-row gap-4">
              {/* Week Selector */}
              <Select value={selectedWeek} onValueChange={setSelectedWeek}>
                <SelectTrigger className="bg-black border-gray-700 text-white rounded-xl py-6 w-48 transition-all duration-300 hover:border-gray-500 focus:border-[#FF4131] focus:ring-[#FF4131] focus:shadow-[0_0_10px_rgba(255,65,49,0.3)]">
                  <div className="flex items-center">
                    <Calendar className="h-4 w-4 mr-2" />
                    <SelectValue placeholder="Select Week" />
                  </div>
                </SelectTrigger>
                <SelectContent className="bg-black border-gray-700 text-white rounded-xl overflow-hidden">
                  <SelectItem value="total" className="focus:bg-gray-800 focus:text-white">
                    Total Points
                  </SelectItem>
                  {weeks.map((week) => (
                    <SelectItem key={week} value={`week${week}`} className="focus:bg-gray-800 focus:text-white">
                      Week {week}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>

              <Button
                onClick={refreshData}
                className="bg-gray-800 hover:bg-gray-700 text-white border border-gray-700 rounded-full px-6 transition-all duration-300 hover:shadow-[0_0_15px_rgba(255,65,49,0.5)] hover:border-[#FF4131]"
                disabled={isLoading}
              >
                <RefreshCw className={`h-4 w-4 mr-2 ${isLoading ? "animate-spin" : ""}`} />
                {isLoading ? "Refreshing..." : "Refresh Data"}
              </Button>
            </div>
          </div>

          <div className={`flex flex-col md:flex-row gap-4 mb-8 items-end ${showAnimation ? "animate-slideUp" : ""}`}>
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
                <Input
                  type="search"
                  placeholder="Search participants..."
                  className="pl-10 py-6 bg-black border-gray-700 text-white rounded-xl focus:border-[#FF4131] focus:ring-[#FF4131] transition-all duration-300 focus:shadow-[0_0_10px_rgba(255,65,49,0.3)]"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
            </div>
            <div className="w-full md:w-48">
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="bg-black border-gray-700 text-white rounded-xl py-6 transition-all duration-300 hover:border-gray-500 focus:border-[#FF4131] focus:ring-[#FF4131] focus:shadow-[0_0_10px_rgba(255,65,49,0.3)]">
                  <SelectValue placeholder="Sort by" />
                </SelectTrigger>
                <SelectContent className="bg-black border-gray-700 text-white rounded-xl overflow-hidden">
                  <SelectItem value="score">Score</SelectItem>
                  <SelectItem value="time">Time</SelectItem>
                  <SelectItem value="date">Date</SelectItem>
                  <SelectItem value="name">Name</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>

          <div
            className={`border border-gray-700 rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.5)] transition-all duration-500 ${showAnimation ? "animate-fadeIn" : ""}`}
          >
            <Table>
              <TableHeader>
                <TableRow className="border-gray-700 bg-gradient-to-r from-gray-900 to-black">
                  <TableHead className="text-white w-16 text-center rounded-tl-2xl py-4">Rank</TableHead>
                  <TableHead
                    className="text-white cursor-pointer hover:text-[#FF4131] transition-colors py-4"
                    onClick={() => handleSortClick("name")}
                  >
                    Participant {getSortIndicator("name")}
                  </TableHead>
                  <TableHead
                    className="text-white text-right cursor-pointer hover:text-[#FF4131] transition-colors py-4"
                    onClick={() => handleSortClick("score")}
                  >
                    {selectedWeek === "total" ? "Total Points" : "Score"} {getSortIndicator("score")}
                  </TableHead>
                  <TableHead
                    className="text-white text-right cursor-pointer hover:text-[#FF4131] transition-colors py-4"
                    onClick={() => handleSortClick("time")}
                  >
                    Time {getSortIndicator("time")}
                  </TableHead>
                  <TableHead
                    className="text-white text-right cursor-pointer hover:text-[#FF4131] transition-colors py-4 rounded-tr-2xl"
                    onClick={() => handleSortClick("date")}
                  >
                    Date {getSortIndicator("date")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentItems.length > 0 ? (
                  currentItems.map((entry, index) => {
                    const position = indexOfFirstItem + index
                    return (
                      <TableRow
                        key={entry.id}
                        className={`border-gray-700 
                          ${index % 2 === 0 ? "bg-black" : "bg-gray-900/30"} 
                          ${position < 3 ? "font-semibold" : ""}
                          ${highlightedRow === entry.id ? "bg-gray-800" : ""}
                          transition-all duration-300 hover:bg-gray-800
                          ${position === 0 ? "hover:shadow-[inset_0_0_20px_rgba(255,215,0,0.1)]" : ""}
                          ${position === 1 ? "hover:shadow-[inset_0_0_20px_rgba(192,192,192,0.1)]" : ""}
                          ${position === 2 ? "hover:shadow-[inset_0_0_20px_rgba(205,127,50,0.1)]" : ""}
                          ${showAnimation ? `animate-fadeIn animation-delay-${index * 100}` : ""}
                        `}
                        onMouseEnter={() => setHighlightedRow(entry.id)}
                        onMouseLeave={() => setHighlightedRow(null)}
                      >
                        <TableCell className="text-center py-4">
                          <div className="flex justify-center items-center">{getRankIcon(index)}</div>
                        </TableCell>
                        <TableCell className="py-4">
                          {position === 0 && (
                            <span className="text-yellow-500 relative">
                              {entry.name}
                              <Sparkles className="h-4 w-4 absolute -right-6 top-1 text-yellow-500 animate-pulse" />
                            </span>
                          )}
                          {position === 1 && <span className="text-gray-300">{entry.name}</span>}
                          {position === 2 && <span className="text-amber-700">{entry.name}</span>}
                          {position > 2 && entry.name}
                        </TableCell>
                        <TableCell className="text-right font-semibold py-4">
                          {position === 0 && <span className="text-yellow-500">{entry.score}</span>}
                          {position === 1 && <span className="text-gray-300">{entry.score}</span>}
                          {position === 2 && <span className="text-amber-700">{entry.score}</span>}
                          {position > 2 && entry.score}
                        </TableCell>
                        <TableCell className="text-right py-4">{entry.time}</TableCell>
                        <TableCell className="text-right py-4">{new Date(entry.date).toLocaleDateString()}</TableCell>
                      </TableRow>
                    )
                  })
                ) : (
                  <TableRow className="border-gray-700">
                    <TableCell colSpan={5} className="text-center py-8">
                      No results found. Try a different search.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
          </div>

          {totalPages > 1 && (
            <Pagination className="mt-8">
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
                    href="#"
                    onClick={(e) => {
                      e.preventDefault()
                      if (currentPage > 1) setCurrentPage(currentPage - 1)
                    }}
                    className={`${currentPage === 1 ? "pointer-events-none opacity-50" : ""} bg-black border-gray-700 text-white hover:bg-gray-900 rounded-xl transition-all duration-300 hover:shadow-[0_0_10px_rgba(255,65,49,0.3)]`}
                  />
                </PaginationItem>

                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  let pageToShow
                  if (totalPages <= 5) {
                    pageToShow = i + 1
                  } else if (currentPage <= 3) {
                    pageToShow = i + 1
                  } else if (currentPage >= totalPages - 2) {
                    pageToShow = totalPages - 4 + i
                  } else {
                    pageToShow = currentPage - 2 + i
                  }

                  return (
                    <PaginationItem key={pageToShow}>
                      <PaginationLink
                        href="#"
                        onClick={(e) => {
                          e.preventDefault()
                          setCurrentPage(pageToShow)
                        }}
                        isActive={currentPage === pageToShow}
                        className={`${currentPage === pageToShow ? "bg-[#FF4131] border-[#FF4131] shadow-[0_0_15px_rgba(255,65,49,0.5)]" : "bg-black border-gray-700"} text-white hover:bg-gray-900 rounded-xl transition-all duration-300`}
                      >
                        {pageToShow}
                      </PaginationLink>
                    </PaginationItem>
                  )
                })}

                {totalPages > 5 && currentPage < totalPages - 2 && (
                  <PaginationItem>
                    <PaginationEllipsis className="text-gray-400" />
                  </PaginationItem>
                )}

                <PaginationItem>
                  <PaginationNext
                    href="#"
                    onClick={(e) => {
                      e.preventDefault()
                      if (currentPage < totalPages) setCurrentPage(currentPage + 1)
                    }}
                    className={`${currentPage === totalPages ? "pointer-events-none opacity-50" : ""} bg-black border-gray-700 text-white hover:bg-gray-900 rounded-xl transition-all duration-300 hover:shadow-[0_0_10px_rgba(255,65,49,0.3)]`}
                  />
                </PaginationItem>
              </PaginationContent>
            </Pagination>
          )}

          <div className={`mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 ${showAnimation ? "animate-slideUp" : ""}`}>
            <div className="border border-gray-700 rounded-2xl p-6 bg-gradient-to-br from-gray-900/50 to-black transition-all duration-300 hover:shadow-[0_0_20px_rgba(255,215,0,0.2)] hover:border-yellow-500/30 group">
              <h2 className="text-xl font-bold mb-4 flex items-center">
                <Trophy className="h-5 w-5 mr-2 text-yellow-500 group-hover:animate-bounce" />
                <span className="text-[#FF4131] group-hover:text-yellow-500 transition-colors duration-300">
                  {selectedWeek === "total" ? "Top Total Points" : "Top Score"}
                </span>
              </h2>
              <div className="text-4xl font-bold text-white group-hover:text-yellow-500 transition-colors duration-300">
                {topScore}
              </div>
              <p className="text-gray-400 mt-2">{topScorer}</p>
            </div>

            <div className="border border-gray-700 rounded-2xl p-6 bg-gradient-to-br from-gray-900/50 to-black transition-all duration-300 hover:shadow-[0_0_20px_rgba(255,65,49,0.2)] hover:border-[#FF4131]/30 group">
              <h2 className="text-xl font-bold mb-4 flex items-center">
                <Award className="h-5 w-5 mr-2 group-hover:animate-pulse" />
                <span className="text-[#FF4131] transition-colors duration-300">
                  {selectedWeek === "total" ? "Average Points" : "Average Score"}
                </span>
              </h2>
              <div className="text-4xl font-bold text-white group-hover:text-[#FF4131] transition-colors duration-300">
                {averageScore}
              </div>
              <p className="text-gray-400 mt-2">
                {selectedWeek === "total"
                  ? `Across ${calculateTotalPoints().length} participants`
                  : `Across ${participantsOnLatestDate} participants`}
              </p>
            </div>

            <div className="border border-gray-700 rounded-2xl p-6 bg-gradient-to-br from-gray-900/50 to-black transition-all duration-300 hover:shadow-[0_0_20px_rgba(52,168,83,0.2)] hover:border-green-500/30 group">
              <h2 className="text-xl font-bold mb-4 flex items-center">
                <Calendar className="h-5 w-5 mr-2 group-hover:animate-pulse" />
                <span className="text-[#FF4131] group-hover:text-green-500 transition-colors duration-300">
                  {selectedWeek === "total" ? "All Weeks" : `Week ${selectedWeek.replace("week", "")}`}
                </span>
              </h2>
              <div className="text-4xl font-bold text-white group-hover:text-green-500 transition-colors duration-300">
                {selectedWeek === "total"
                  ? weeks.length
                  : latestDate.toLocaleDateString("en-US", { month: "short", day: "numeric" })}
              </div>
              <p className="text-gray-400 mt-2">
                {selectedWeek === "total" ? "Total weeks completed" : `${participantsOnLatestDate} participants`}
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

