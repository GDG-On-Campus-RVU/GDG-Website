...
function TeamPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      Object.keys(teamsData).forEach(team => {
        ...
        if (rect && rect.top <= window.innerHeight * 0.3 && rect.bottom >= window.innerHeight * 0.7) {
          setActiveTeam(team);
        }
      });
    };
  ...
  const scrollToTeam = (teamName) => {
    document.getElementById(teamName)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
  ...
  return (
    <TemplatePage>
      <div className="fixed top-0 z-50 bg-black w-full flex gap-2 px-4 py-3">
        {Object.keys(teamsData).map((team, i) => (
          <button
            key={team}
            onClick={() => scrollToTeam(team)}
            className={`w-4 h-4 rounded-full transition-colors duration-300 ${
              activeTeam === team ? `bg-${teamColors[i]}-500` : "bg-gray-300"
            }`}
          />
        ))}
      </div>
      ...
      <div ref={containerRef} className="h-screen overflow-y-auto snap-y snap-mandatory">
        {Object.entries(teamsData).map(([team, members], i) => (
          <section
            key={team}
            id={team}
            ...
            <div className="flex flex-col lg:flex-row gap-12 text-white">
              <div className="flex-1">
                <motion.h2
                  ...
                  className={`sticky top-12 text-4xl font-bold text-${teamColors[i]}-500 mb-4 z-10 bg-black py-2`}
                >
                  {team}
                </motion.h2>
